...
function BookingDateTimePage({ doctorId }) {
  ...
  useEffect(() => {
    ...
    axios.get(`/api/doctor/${doctorId}/slots?date=${d}`)
      .then(res => {
...
//   const [timeSlots, setTimeSlots] = useState([]);
// const doctorId = 1; // Replace with dynamic doctor ID later if needed
...
//       const res = await axios.get(`/api/doctor/${doctorId}/slots?date=${formattedDate}`);
...
// const slotsForDay = timeSlots.filter(slot => slot.isActive);
