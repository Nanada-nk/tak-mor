...
function BookingPage() {
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://localhost:9090/api/doctor/${doctorId}`)
      .then(res => {
        setDoctor(res.data);
        const firstSpecialty = res.data.specialties?.[0]?.Specialty?.name;
        if (firstSpecialty && !selectedSpecialty) {
          setLocalSpecialty(firstSpecialty);
          setSpecialty(firstSpecialty);
        }
      })
      .catch(err => console.error("Failed to fetch doctor:", err));
  }, [doctorId, selectedSpecialty, setSpecialty]);
console.log('doctor!!!!!!!!!', doctor)
  ...
  const services = useMemo(() => {
    return [
      ...
      { name: "Mental Health Checkup", price: 900 },
      { name: "Heart Screening", price: 1500 },
      { name: "Skin Evaluation", price: 800 },
    ];
  }, []);
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6">
      ...
      <div className="my-6">
        <h2 className="text-lg font-semibold mb-2">Select Specialty</h2>
        <SpecialtySelector
