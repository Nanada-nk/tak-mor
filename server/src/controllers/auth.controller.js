...
// ฟังก์ชันกลางสำหรับจัดการเมื่อ Login สำเร็จ (สำหรับ Local Login)

authController.loginSuccess = async (req, res, next) => {
  try {
    // req.user ถูกส่งมาจาก Passport strategy หลังจากยืนยันตัวตนสำเร็จ
    const user = req.user;
    ...
    // 4. ตั้งค่า Refresh Token ใน httpOnly Cookie
    res.cookie('refreshToken', refreshToken, {
      httpOnly: true,
      secure: process.env.NODE_ENV === 'production',
      sameSite: 'strict', // ป้องกัน CSRF ได้ระดับหนึ่ง
...
// Handler สำหรับ Social Login Success
authController.socialLoginSuccess = async (req, res, next) => {
  try {
    // req.user ถูกส่งมาจาก Passport strategy ของ Google/Facebook
    const user = req.user;

    // สร้าง Access Token และ Refresh Token เหมือนกับ Local Login
    const accessToken = await jwtService.genAccessToken({ id: user.id, role: user.role });
    const refreshToken = await jwtService.genRefreshToken(user.id);
    ...
    // Redirect กลับไปหา Frontend พร้อมกับ Access Token เพื่อให้ Client จัดการต่อ
    res.redirect(`${process.env.FRONTEND_URL}/auth/callback?token=${accessToken}`);
  } catch (err) {
    // หากเกิดข้อผิดพลาด ให้ redirect กลับไปหน้า login พร้อม error message
