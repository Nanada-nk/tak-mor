...
//     const { roomId, userId, userName, callType } = payload;
//     console.log(`User ${userName} (${userId}) joining room ${roomId} for ${callType} call.`);

//     if (!rooms[roomId]) {
//       rooms[roomId] = {};
//     }
//     rooms[roomId][userId] = { socketId: socket.id, userName, callType };
//     socketToUserMap[socket.id] = { roomId, userId };

//     socket.join(roomId);

//     const otherUsersInRoom = Object.keys(rooms[roomId]).filter(id => id !== userId);

//     if (otherUsersInRoom.length > 0) {
//       const otherUserId = otherUsersInRoom[0];
//       const otherUserSocketId = rooms[roomId][otherUserId].socketId;
//       const otherUserName = rooms[roomId][otherUserId].userName;

//       console.log(`Room ${roomId} already has user ${otherUserName}. Signaling new user.`);

//       socket.to(otherUserSocketId).emit('user-joined', {
//         userId: userId,
//         userName: userName,
//         socketId: socket.id
//       });
...
//     } else {
//       console.log(`User ${userName} is the first in room ${roomId}.`);
//     }
//   });
...
//     const { appointmentId } = msg; // msg: { appointmentId, senderId, senderName, content, timestamp, isRead }
//     console.log(`Chat message in room ${appointmentId} from ${msg.senderName}: ${msg.content}`);
//     // ส่งข้อความกลับไปหา Client ทุกคนในห้องที่เกี่ยวข้องกับ appointmentId นี้
//     io.to(appointmentId).emit('receive_message', msg); // Broadcast to all in the chat room
//   });
